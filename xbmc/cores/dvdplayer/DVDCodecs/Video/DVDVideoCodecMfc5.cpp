...
#undef CLASSNAME
#endif
#define CLASSNAME "CDVDVideoCodecMfc5"

CDVDVideoCodecMfc5::CDVDVideoCodecMfc5() : CDVDVideoCodec() {
...
CDVDVideoCodecMfc5::~CDVDVideoCodecMfc5() {
  Dispose();
}

bool CDVDVideoCodecMfc5::OpenDevices() {
  DIR *dir;
  struct dirent *ent;
...
bool CDVDVideoCodecMfc5::Open(CDVDStreamInfo &hints, CDVDCodecOptions &options) {
...
void CDVDVideoCodecMfc5::Dispose() {
...
void CDVDVideoCodecMfc5::SetDropState(bool bDrop) {
...
int CDVDVideoCodecMfc5::Decode(BYTE* pData, int iSize, double dts, double pts) {
  ...
  //printf("%s::%s - input frame pts %lf, dts %lf\n", CLASSNAME, __func__, pts, dts);
  ...
  if (m_bDropPictures) {
    ...
    m_iFIMCdequeuedBufferNumber = CLinuxV4l2::DequeueBuffer(m_iConverterHandle, V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE, V4L2_MEMORY_MMAP, V4L2_NUM_MAX_PLANES, &dequeuedTimestamp);
    ...
    m_v4l2FIMCCaptureBuffers[m_iFIMCdequeuedBufferNumber].timestamp = dequeuedTimestamp;
#endif
...
#endif
    m_videoBuffer.pts = m_v4l2FIMCCaptureBuffers[m_iFIMCdequeuedBufferNumber].timestamp;
  ...
  //printf("%s::%s - output frame pts %f", CLASSNAME, __func__, m_videoBuffer.pts);
...
bool CDVDVideoCodecMfc5::GetPicture(DVDVideoPicture* pDvdVideoPicture) {
...
bool CDVDVideoCodecMfc5::ClearPicture(DVDVideoPicture* pDvdVideoPicture)
{
